...
Use [minimind_colab_learning.ipynb](minimind_colab_learning.ipynb) from top to bottom on a BF16-capable Google Colab GPU. An A100 is recommended for the full Zero route; an L4 is supported for the complete micro route and a conservative Zero learning run. The notebook is deliberately ordered so that each conceptual checkpoint has a real command behind it:
...
The notebook accepts any CUDA GPU that reports BF16 support by default. Keep `REQUIRE_A100 = False` for an L4; set it to `True` only when A100-specific validation matters. An L4 with 22 GB can run the full micro route. The optional Zero route starts at `ZERO_BATCH_SIZE = 8` to leave memory headroom; raise it only after a stable run. This L4-safe setting lowers the effective batch relative to the repository's default profile, so treat it as a learning-oriented variant rather than an exact reference reproduction.
...
The notebook's repository/setup cell is a readable multi-line `%%bash` block with `set -euo pipefail`, without Python control flow or `subprocess`. The preceding configuration cell exports the four values Bash needs. The block clones only when `ROOT` is absent; otherwise it verifies that `ROOT` is a Git checkout, fetches `REPOSITORY_REF`, and checks out the fetched revision. It then enters the repository and runs setup, so a Git failure cannot fall through into setup. Later cells use absolute runner/root arguments and do not depend on notebook working-directory state. Existing `.colab-venv`, downloaded data, checkpoints, and outputs are not deleted.
